...
//         if (data.success) {
//             this.summaries = data.summaries;
//             this.discordConfigured = data.discord_configured || false; // Add this line
//             this.successMessage = 'Social media summaries generated successfully!';
//         } else {
//             this.error = data.error || 'Failed to generate summaries';
//         }
...
//                 //         if (data.success) {
//                 //             this.successMessage = `Post scheduled for ${platform} at ${new Date(this.scheduleTime).toLocaleString()}!`;
...
//                 //         this.successMessage = `Scheduled posts for ${scheduledPlatforms.join(', ')} at ${new Date(this.scheduleTime).toLocaleString()}!`;
//                 //         this.showScheduleModal = false;
//                 //         // Reset selections
//                 //         for (const platform in this.selectedPlatforms) {
...
//                 //         }
//                 //     } else {
//                 //         this.error = 'Failed to schedule any posts';
//                 //     }
//                 // },
//                 async scheduleMultiplePosts() {
//     if (!this.videoId) {
...
function app() {
    return {
        ...
        initAuth() {
            try {
                // Check if a video was selected from search page
                const savedVideoUrl = localStorage.getItem('selectedVideoUrl');
                const savedVideoTitle = localStorage.getItem('selectedVideoTitle');

                if (savedVideoUrl) {
                    this.youtubeUrl = savedVideoUrl;
                    this.successMessage = `Video selected: ${savedVideoTitle || 'Loading...'}`;

                    // Clear the saved URL so it doesn't auto-populate again
                    localStorage.removeItem('selectedVideoUrl');
                    localStorage.removeItem('selectedVideoTitle');

                    // Auto-trigger transcript extraction after a brief moment
                    setTimeout(() => {
                        this.successMessage = '';
                        this.getTranscript();
                    }, 1000);
